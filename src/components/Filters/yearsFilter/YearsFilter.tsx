...
const YearsFilter = ({
  ...
  const openList = (e: any) => {
    ...
    if (isActive) {
      ...
      refInputText.current ? refInputText.current.style.backgroundColor = `#312b45` : '';
      if (e.target.classList[0] == styles.selectBoxTitle || e.target.classList[0] == styles.selectBoxExtra || e.target.classList[2] == styles.selectBoxIcon) {
        list = e.target.parentElement.parentElement.parentElement.parentElement.parentElement.children;
      } else {
      ...
      refInputText.current ? refInputText.current.style.backgroundColor = `#7e798f` : '';
      if (e.target.classList[0] == styles.selectBoxTitle || e.target.classList[0] == styles.selectBoxExtra || e.target.classList[2] == styles.selectBoxIcon) {
        list = e.target.parentElement.parentElement.parentElement.parentElement.parentElement.children;
      } else {
        list = e.target.parentElement.parentElement.parentElement.parentElement.children;
  ...
  const addChecked = (e: any) => {
    ...
    if (e.target.textContent == 'Все годы' || e.target.textContent == 'All years') {
      setSearchParams({ ...params, year: 'all' })
    } else if (e.target.textContent.length == 8 || e.target.textContent.length == 9) {
  ...
  return (
    <>
      <div className={styles.selectBox}>
        <div className={styles.selectBoxCurrent} tabIndex={1} onClick={openList}>
          <div className={styles.selectBoxValue}>
            ...
            <div className={styles.selectBoxInputText} ref={refInputText} >
              ...
              <FontIcon appearance="dropDownArrow" className={clsx(styles.selectBoxIcon, isActive ? styles.active : '')} />
            </div>
          </div>

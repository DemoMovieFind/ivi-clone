...
const FilmWatchCard = ({
  ...
  const btnTrailerContent = <div className={styles.btnTrailerContent}>
    <FontIcon appearance='play' /><FormattedMessage id='film_watch_trailer' />
  </div>

  let currentFilmType = ''
  film?.type == 'фильм' ? currentFilmType = 'movies' :
    ...
    <div className={styles.filmWatchCardContainer}>
      <nav className={styles.filmWatchCardBreadCrumbsContainer}>
        ...
        <Link to={`/${currentFilmType}/${film?.genre[0]}`} className={styles.breadCrumbsItem}>
          {currentFilmType == 'animations' ?
            film?.genre && film?.genre[1].slice(0, 1).toUpperCase() + film?.genre[1].slice(1)
            :
        ...
        <div className={styles.filmWatchInfoSide}>
          <div className={styles.filmWatchTitle}>
            {currentFilmType == 'movies' ?
              ...
              currentFilmType == 'series' ?
                `${film?.name} 
              (${intl.formatMessage({ id: 'film_watch_series' }).slice(0, -2)}al 
              ${film?.year})`
                :
                currentFilmType == 'animations' ?
                  `${film?.name.split(' ').slice(0, -1).join(' ')} 
              (${intl.formatMessage({ id: 'film_watch_cartoons' }).slice(0, -1)} 
              ${film?.year})`
                  :
                  ''
            }
          ...
          <div className={styles.filmWatchParamsContainer}>
            <div className={styles.filmWatchMovieInfo}>
              ...
              {film?.time && film?.time?.length <= 7 ? ` ${film?.time}.`
                : `
            ...
            <nav className={styles.filmWatchGenreNav}>
              {film?.country.map((country, index) => {
                return <Link
                  key={index}
                  className={clsx(styles.filmWatchGenreNavCountry, styles.breadCrumbsItem)}
              ...
              {film?.genre.map((genre, index) => {
                return <Link
                  ...
                  key={index}
                  to={`/movies?genres=${genre.slice(0, 1).toUpperCase() + genre.slice(1)}`}>
          ...
          <div className={styles.filmWatchActorsContainer}>
            ...
            {film?.actors && film?.actors.slice(0, 4).map((actor, index) => {
              return <NavLink to={`/persons/${actor}`} state={actor} key={index}>
                <PersonCardMini name={actor} img='https://thumbs.dfs.ivi.ru/storage38/contents/b/c/45102370a23e374f4146fe2d106f26.jpeg/88x88/?q=85' />
              </NavLink>
            })}
          </div>
